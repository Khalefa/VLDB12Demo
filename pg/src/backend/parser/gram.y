 ...
 */
timeseries_range: '[' Iconst ',' Iconst ']' 
		{
			PastFutureRange *n= makeNode(PastFutureRange);
			n->s=$2;
			n->e=$4;
			$$= (Node *) n;		
		} | {$$=NIL;
...
func_name:	type_function_name
					{setGrpFnc($1); $$ = list_make1(makeString($1)); }
			| relation_name indirection
...
void setGrpFnc(char* s){
//elog(WARNING,"setgrpFnc %c %c ",s[0],s[1]);
//return; 
if(s[0]=='s')grp_fnc='s';
else if(s[0]=='S')grp_fnc='s';
else if(s[0]=='a')grp_fnc='a';
else if(s[0]=='A')grp_fnc='a';
